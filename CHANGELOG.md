...
### Changed
- **Strict Typing** - Replaced `any` with specific types like `MessageContent`, `PaginatorContext`, and `PaginatorOptions` for better type safety.
- **Improved Error Handling** - Enhanced error handling in `send` and `update` methods to catch and emit errors properly.
- **Refactoring** - Cleaned up the `Paginator` class to use the new types and improve code readability.
